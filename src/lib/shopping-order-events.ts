...
// (PR6)を介した場合に限定する。
export async function recordShoppingOrderEvent(eventId: string, eventType: string, body: Record<string, unknown>): Promise<void> {
  ...
  if (insertError) {
    if (insertError.code === "23505") return; // 同一event_idは記録済み(冪等)。
    throw insertError;
  }
}
