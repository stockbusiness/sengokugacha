// リポジトリに存在するマイグレーションのversion一覧。
//
// 実行時にこの一覧とDBの supabase_migrations.schema_migrations を突き合わせて、
// staging/本番への適用漏れを検知する(/admin/operations-health)。
//
// なぜ実行時にディレクトリを読まずに定数で持つのか:
// supabase/migrations/ はNext.jsのビルド成果物に含まれないため、デプロイ先で
// ファイル一覧を読める保証が無い。読めなかったときに「全件未適用」と誤検知すると、
// 安全網としてかえって有害なので、確実に読める定数にしている。
//
// 手で足す必要があるが、忘れると expected-migrations.test.ts がCIで落ちて
// 「どのversionを足せばよいか」まで教えてくれる。マイグレーションを追加したら
// ここに1行足すこと。
export const EXPECTED_MIGRATION_VERSIONS: string[] = [
  ...
  "20260812000001", // castle_lord_plan_price_override
  "20260813000001", // agency_project_key_and_referral_token
  ...
  "20260821000001", // product_ownership
  "20260822000001", // payout_exclusivity
];
