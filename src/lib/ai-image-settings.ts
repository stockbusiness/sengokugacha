...
- 最優先事項: 画面全体が均一な明るさの「のっぺりした」仕上がりを避け、強いコントラストで立体感を出すこと。片側上方からのスポットライトのような強い一方向の光源を設定し、光が直接当たる面(兜の頂点・肩当て・胸の金属装飾など)には白に近い鋭いハイライトを、反対側の面には黒に近い深い影を作る。同じ鎧でも面の向きによって明暗がはっきり変わるように描き分ける
- 人物の輪郭(肩・兜・腕)に沿って、背景よりも明るい細い縁光(リムライト)を入れて人物を背景から浮き立たせる。背景は人物より彩度と明度を落とし、輪郭をやや甘くぼかして奥行きの層を作る(人物だけが手前にくっきり浮かぶ構図)
- 鎧の金属装飾の継ぎ目・重なり部分には小さな陰影を描き込み、金属が持つ厚み・丸みを感じさせる
...
- 上記の強いコントラストを優先しつつ、人物の顔・表情だけははっきりと見える明るさを保った、シネマティックで高級感のある仕上がり`;
