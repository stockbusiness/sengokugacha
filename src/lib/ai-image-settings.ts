...
- 鎧の金属装飾には強いハイライトと反射光を入れ、金箔や磨かれた金属のような質感の高さを表現する
- 平面的でのっぺりした塗りは避け、強い立体感を出す。片側からの強いキーライトに加え、輪郭を浮かび上がらせる逆光気味のリムライト(縁光)を入れ、鎧のプレートの継ぎ目・重なり・しわの一つ一つに落ちる細かな陰影(アンビエントオクルージョン)まで丁寧に描き込み、金属や布に厚み・量感を持たせる
- 人物にピントを合わせ、背景は被写界深度でぼかして奥行きを出す(前景・人物・背景の3層で空間の層を作り、人物をくっきりと浮き立たせる)
