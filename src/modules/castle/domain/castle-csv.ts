...
// ============================================================
//
// まだ1件も登録が無い状態では「まずエクスポートして形式を見る」ができないため、
// 列の並びと各列に何を書くのかが分かる記入例を配布する。
//
// 城・区画とも管理画面に削除機能が無く、誤って取り込むと消せないため、
// 名称には「(記入例)」を付けて、取り込まれてしまった場合でも一目で分かるようにしている。
//
// CASTLE_CSV_HEADER / PLOT_CSV_HEADER と列数・列順が一致していること、および
// 取り込み時の検証を通過することはテストで保証している(記入例が仕様から
// ずれると、それを信じて書いた利用者が必ずエラーになるため)。

export const CASTLE_CSV_SAMPLE_ROWS: string[][] = [
  // 城主募集中 + 城ごとの料金上書き + 国の制圧で解放、の全部入り。
  [
    "",
    "(記入例)岐阜城",
    "岐阜県",
  ...
  // 公開中 + 料金は共通設定に任せる(空欄)。
  ["", "(記入例)犬山城", "愛知県", "中部", "published", "PUBLIC", "unreviewed", "2", "", "現存天守を持つ城。", "", ""],
  // 最小限。城名以外を空にした場合に既定値がどうなるかの例。
  ["", "(記入例)小田原城", "神奈川県", "関東", "draft", "PUBLIC", "unreviewed", "3", "", "", "", ""],
];

export const PLOT_CSV_SAMPLE_ROWS: string[][] = [
  ["", "SAMPLE-001", "(記入例)一番区画", "一番街区", "300000", "available", "1", "大手門に近い区画。", ""],
  ["", "SAMPLE-002", "(記入例)二番区画", "一番街区", "350000", "available", "2", "", ""],
  // 街区を分けた例と、まだ販売開始しない下書きの例。
  ["", "SAMPLE-003", "(記入例)三番区画", "二番街区", "280000", "draft", "3", "", ""],
];
