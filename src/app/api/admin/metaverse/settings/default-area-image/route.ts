...
export async function POST(request: NextRequest) {
  ...
  const path = `defaults/area-${Date.now()}.${resized.extension}`;
  ...
  if (existing?.id) {
    result = await supabase
      .from("metaverse_tour_settings")
      .update({ default_area_image_url: publicUrl, updated_at: new Date().toISOString() })
      .eq("id", existing.id)
      .select("*")
      .single();
  } else {
    result = await supabase
      .from("metaverse_tour_settings")
      .insert({ default_area_image_url: publicUrl })
      .select("*")
      .single();
  }
  if (result.error) {
  ...
  await logAdminAction(await getAdminActorName(), "metaverse_default_area_image_update");
...
export async function DELETE() {
  ...
  if (!existing?.id) {
    return NextResponse.json({ id: null, default_area_image_url: null });
  }

  const result = await supabase
    .from("metaverse_tour_settings")
    .update({ default_area_image_url: null, updated_at: new Date().toISOString() })
    .eq("id", existing.id)
    .select("*")
    .single();
  if (result.error) {
  ...
  await logAdminAction(await getAdminActorName(), "metaverse_default_area_image_clear");
  return NextResponse.json(result.data);
}
