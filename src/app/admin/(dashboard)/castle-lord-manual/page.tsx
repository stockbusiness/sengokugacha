...
const SECTIONS: Section[] = [
      ...
      <>
        ...
        <ol className="list-decimal space-y-1 pl-5">
          ...
          <li>
            ...
            城主契約の有効化に合わせて販売可能になる。代理店が紹介URL・QRを発行し、購入者がアプリから区画を購入する。
          </li>
          <li>
            <strong>報酬の計上〜支払</strong>(「土地報酬元帳」「土地報酬支払」ページ): 区画の購入が成立すると
            自動的に報酬が計上され、一定期間(既定8日)経過後に本部が確定操作を行い、支払処理を行う。
          </li>
        </ol>
      </>
    ),
  ...
  },
  {
    ...
    body: (
      <>
        ...
        <ul className="list-disc space-y-1 pl-5">
          <li>紹介URLはLINEアプリ内で該当の区画詳細ページを直接開けるリンクです。誰が販売したかが自動的に記録されます。</li>
  ...
  },
  {
    ...
    body: (
      <>
        <p>購入者(一般ユーザー)は次の流れで区画を購入します。</p>
        <ol className="list-decimal space-y-1 pl-5">
          <li>「全国のお城一覧」(<code>/castles</code>)から城・区画を選ぶ(または代理店の紹介URLから直接区画詳細ページを開く)。</li>
          <li>区画詳細ページで「購入手続きへ進む」を押すと、その区画が一時的に予約される(既定24時間、他の人は購入できなくなる)。</li>
          <li>Stripeの決済ページで支払う。決済完了と同時に区画の所有権が確定し、「所有区画」ページ(<code>/my-land</code>)に反映される。</li>
          <li>予約から一定時間内に支払われなかった場合、予約は自動的に失効し、区画は再び販売可能に戻る(次に誰かがアクセスした時点で処理されます)。</li>
        </ol>
      </>
    ),
  },
      ...
      <>
        ...
        <ol className="list-decimal space-y-1 pl-5">
          <li>
            ...
            下書き作成は本部担当者でも可能ですが、<strong>公開(実際の計算に使われるようにする操作)は本部管理者のみ</strong>実行できます。
            公開済みのルールは編集できません(修正したい場合は新しいルールセットを作って公開し直してください)。
          </li>
          <li>
