-- ============================================================
-- 0. 安全ゲート。0件でなければ適用しない(ご指示による2回目の確認)
-- ============================================================
do $$
declare v_before int;
begin
  select count(*) into v_before from entitlements;
  if v_before <> 0 then
    raise exception
      '適用を中止しました。entitlements が % 件あります。自動更新は行いません。既存行を集計のうえ移行計画を提出します。', v_before;
  end if;
end $$;

create temp table __p2b_report as
select (select count(*) from entitlements) as before_entitlements;
...
-- ============================================================
--
-- 判定の確認は関数を直接呼ぶだけで、行を作らない。
-- 残高の確認だけは実際に付与・取消を通す必要があるため1行だけ作り、必ず削除する。
do $$
declare
  v_user_id uuid;
...
begin
  -- 検証用の送信元。最後に必ず消す。
  insert into entitlement_source_allowlist (source_system_key, note, approved_by)
  values ('__verify__', '適用検証用。このスクリプト内で削除される', 'verify');

  alter table __p2b_report
    add column no_code text,
    add column mismatch text,
    add column not_owned text,
    add column ok text,
    add column land_plot text,
    add column bad_grant_balance text,
    add column grant_revoke text;

  -- 判定の確認(行を作らない)
  update __p2b_report set
    no_code   = entitlement_application_decision('__verify__', null,               'kokudaka'),
    mismatch  = entitlement_application_decision('__verify__', 'SPPT_KOKUDAKA',    'gacha_ticket'),
    not_owned = entitlement_application_decision('__verify__', 'tenka_pass',       'kokudaka'),
    ok        = entitlement_application_decision('__verify__', 'SPPT_KOKUDAKA',    'kokudaka'),
    land_plot = entitlement_application_decision('__verify__', 'SPPT_LAND_PLOT',   'land_plot');

  select id, kokudaka into v_user_id, v_kokudaka_before from users order by created_at limit 1;

  if v_user_id is null then
    update __p2b_report set bad_grant_balance = 'skipped(users 0件)', grant_revoke = 'skipped(users 0件)';
  else
    -- 不正な付与で残高が変化しないこと
    insert into entitlements (entitlement_id, common_user_id, user_id, entitlement_type, product_code, source_system_key, quantity)
    values ('__verify_bad__', '__verify__', v_user_id, 'kokudaka', null, '__verify__', 100)
    returning id into v_bad_id;
    perform process_entitlement_grant(v_bad_id);
    select kokudaka into v_kokudaka_mid from users where id = v_user_id;
    delete from entitlements where id = v_bad_id;

    -- 正常付与 → 取消が同じ商品コード条件で動くこと
    insert into entitlements (entitlement_id, common_user_id, user_id, entitlement_type, product_code, source_system_key, quantity)
    values ('__verify_ok__', '__verify__', v_user_id, 'kokudaka', 'SPPT_KOKUDAKA', '__verify__', 100)
    returning id into v_id;
    perform process_entitlement_grant(v_id);
    select claim_outcome into v_revoke from process_entitlement_revocation(v_id);
    delete from entitlements where id = v_id;

    select kokudaka into v_kokudaka_after from users where id = v_user_id;

    update __p2b_report set
      bad_grant_balance = format('%s → %s', v_kokudaka_before, v_kokudaka_mid),
      grant_revoke = format('取消=%s 国高 %s → %s', v_revoke, v_kokudaka_before, v_kokudaka_after);
  end if;

  delete from entitlement_source_allowlist where source_system_key = '__verify__';
end $$;

insert into supabase_migrations.schema_migrations (version)
values ('20260821000001')
on conflict (version) do nothing;

select
  before_entitlements                       as "1_適用前_entitlements件数",
  (select count(*) from entitlements)       as "2_適用後_entitlements件数",
  '20260821000001_product_ownership'        as "3_適用したマイグレーションID",
  (select count(*) from supabase_migrations.schema_migrations
     where version='20260821000001')        as "4_schema_migrations登録",
  (select count(*) from entitlement_source_allowlist) as "5_allowlist件数",
  no_code                                   as "6_商品コードなし",
  mismatch                                  as "7_コードと種別の不一致",
  not_owned                                 as "7b_担当外コード",
  land_plot                                 as "7c_land_plot",
  bad_grant_balance                         as "8_不正付与での国高変化",
  grant_revoke                              as "9_正常付与と取消",
  (select count(*) from pg_proc p join pg_namespace n on n.oid=p.pronamespace
     where n.nspname='public' and p.proname in ('entitlement_balance_column','entitlement_application_decision')
       and p.pronargs = 2)                  as "10_旧2引数版の残存"
from __p2b_report;
