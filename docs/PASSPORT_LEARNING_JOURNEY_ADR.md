...
- 状態: **提案（承認待ち）**
- 日付: 2026年8月20日（同日、ウォレット側の優先度A回答を反映）
- 前提資料: `docs/PASSPORT_LEARNING_MISSION_CURRENT_STATE.md`、
  `docs/WALLET_INTEGRATION_QUESTIONS.md`、`docs/WALLET_INTEGRATION_ANSWERS_20260820.md`
...
- **本番稼働はしておらず、稼働開始時期も未定**（2026-08-20、ウォレット開発担当より）。
  staging は稼働中だが検証用の位置づけで、総アカウント数は2件（初期管理者関連のみ）
- staging は **`AUTH_MODE=production`**（2026-07-18の実チャネルLINEログイン結合試験を受けて切替済み）。
  **戦国パスポートSSOは引き続きモック実装**
  ※ 当初 `AUTH_MODE=mock` と記載していたが、相手方リポジトリの `docs/deployment.md` が
  実態より古かったため訂正した
...
3. **初期実証は指示書§8.1の選択肢2（`MISSION_REWARDS_ENABLED` OFF）とする。**
   ウォレットが本番稼働していない以上、これは「既定」ではなく**唯一の選択肢**。
   付与要求レコードは作って `PENDING` のまま保留し、ウォレット稼働後に送信できる構造にする。
   **ウォレット側からもこの進め方で問題ないとの回答を得ている**（`ENABLE_*` 系フラグを
   段階的にONにしていく既存の運用パターンと一致するため「望ましい」との評価）
...
## ADR-6: `external_user_id` には `users.id` を送る
...
### ウォレット側の合意 — **取得済み**（2026-08-20）

`docs/WALLET_INTEGRATION_ANSWERS_20260820.md` 1章のとおり:

- `common_user_id` を付与APIが受け付けないという理解は**合っている。追加予定も無い**
- `users.id` を送る案は**妥当**との回答
...
- **一度作成した `external_user_id` をリネームするAPI・機能は存在しない。**
  `AccountLink(serviceIntegrationId, externalUserId)` は一意制約のキーで、
  既存レコードを書き換える運用は想定されていない
- 同一人物に複数アカウントができた場合の復旧は**アカウント統合機能**のみ。
  管理画面からの手動申請 + **別の管理者による承認（二段階承認制、金額によらず常に必要）**。
  パスポート側から自動で呼べるAPIではない

したがって、**送信前の検証を実装側で担保する**:

- `external_user_id` は付与要求レコード作成時に確定して保存し、以後**書き換えない**
- 送信直前に「保存済みの値」と「現在の `users.id`」の一致を確認する
- テスト環境と本番環境で同じ `users.id` が混ざらないよう、`service_integrations` を
  環境ごとに分ける（質問4-2、優先度Bで確認中）

### 共通IDの扱い

指示書§9「共通IDを取得できない場合は付与を行わず進捗を安全に保留する」は、
`external_user_id` に `users.id` を使う場合でも**方針として維持する**
（共通IDが解決できていないユーザーは、他システムとの突合ができないため）。
付与要求レコードには `common_user_id` も併記して保存する。

---

## ADR-7: 既存付与の重複防止は「照会」ではなく「取込」で行う
...
**ウォレット側から確認済み**（2026-08-20）: 取引履歴照会APIを**新設する予定は未定**。
必要なら正式に機能要望として提出すれば検討する、との回答。
当面は§8.4(2)で進めることを推奨された。

さらに、**旧3,000 OVE は1件も確定付与されていない**ことが確認できた。
`wallet_referral_benefits` の `status` を書き込むコードは1箇所のみで、遷移は
`CAPTURED → PENDING` だけ。`CONFIRMED` へ遷移させるコードは存在せず、
「確定付与は構造的に発生し得ない」との回答。staging の総アカウント数も2件（初期管理者関連のみ）。
...
### ★ 帰結: 「既存参加者は3,000 OVE受領済み」という前提を撤回する
...
> `wallet_referral_benefits` の status には反映されず、通常の取引履歴・監査ログにのみ残る。

ウォレット管理画面の「操作ログ」または「取引一覧」で、金額3,000・理由に「紹介」等を含む
手動付与の有無を確認する必要がある。**確認できるまでは既存参加者への新規付与を保留する。**
...
| PR2 | ミッション基盤（11テーブル・ドメイン純粋関数・採点・完了判定・単体テスト）+ `learning_journey_settings`（全フラグOFF）+ ADR-10の監査ログ3列 | — |
| PR3 | 参加者画面5枚 + 参加者向けAPI6本 + 入口（ホーム・SideMenu） | — |
| **PR4（完了）** | 管理画面5枚 + 管理API11本 + 監査ログ表示の拡張 + 不正疑いの検知（表示のみ） | — |
...
| 2 | **OVEウォレットの本番稼働時期**（未定）。および `service_integrations` の `SENGOKU_PASSPORT` 行（APIキー・署名鍵・上限額）の発行 | ウォレット側（優先度B 4章の回答待ち） |
| 2-b | 「はじまりの旅」用の `transaction_type` と `reward_rules` を新設するか、既存の汎用種別を使うか | ウォレット側（優先度B 5章の回答待ち） |
| 3 | CSV一括付与で単発の3,000 OVE手動付与が行われていないかの確認（操作ログ・取引一覧） | 運営 |
| 3-b | §19.1(5)「新制度の対象者判定基準」の定義し直し（3,000 OVE受領を基準にできないため） | 運営 |
