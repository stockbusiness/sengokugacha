...
## Ver2.10追記2: 運用面の不足点対応(フォールバック・履歴・スキル名の事故防止)

ユーザーからの「不足点を教えて」というレビュー依頼を受けて洗い出した、以下の運用上のギャップに対応した。

### 変更内容

- **プロバイダ自動フォールバック**: `src/lib/ai-image.ts`の`generateImage()`が、設定中のプロバイダ(例: OpenAI)が失敗した場合、もう一方のプロバイダ(Gemini)にAPIキーが設定されていれば自動で1回だけ再試行するようにした。レート制限やモデレーション拒否等、プロバイダを変えると成功する可能性があるケースを想定。実際にどちらのプロバイダ・モデルが使われたか(`providerUsed`/`modelUsed`)を返すようにし、`ai_generated_images`の`provider`/`model`列には常に実際に使われた方を記録する(設定上のプロバイダではない)。管理画面のプレビュー時にフォールバックが発生した旨を通知する。
- **スキル名生成の事故防止**: これまで「AIで生成」ボタンを押すと即座にDBへ保存されていた(画像生成が採用ボタンを押すまで保存しないのと非対称だった)。`/api/admin/warlords/[id]/skill-name`に`persist`パラメータを追加し、個別の「AIで生成」ボタンは`persist: false`(欄に反映するのみ、内容を確認・編集してから通常の「保存」ボタンで確定)、一括生成ボタンは`persist: true`(75体を1件ずつ確認する運用が非現実的なため、生成と同時に保存)に変更した。
- **生成履歴・使用状況の可視化**: 管理画面`/admin/ai-image-history`(新規)を追加。`ai_generated_images`の直近200件を対象に、生成回数・採用数・プロバイダ内訳のサマリーと、一覧(エンティティ種別・プロンプト・プロバイダ/モデル・採用状態・日時)を表示する。採用済みの生成のみプレビュー画像を表示できる(未採用分は元々プレビュー時に破棄する設計のため、画像そのものは保存されていない)。金額(コスト)はモデル・サイズで単価が変わり不正確になりやすいため、件数のみを表示する方針にした。`/admin/ai-image-settings`からリンクを追加。

### 影響範囲

- 既存テーブルへの列追加・変更なし(`ai_generated_images`は元々あった列をそのまま使う)。スキル名の個別生成ボタンの挙動が「即保存」から「欄に反映のみ」に変わる点が唯一の破壊的変更(一括生成ボタンの挙動は変わらない)。

### 未実装事項(このレビューで挙げたが今回は対応していないもの)

- 家紋・国章の実グラフィック合成(現状はテキストバッジのみ)。
- 1回の生成で複数案(2〜4枚)を同時に生成して選ばせるUI。
- 内覧シーンの一括生成(現状は1シーンずつ)。
- カード枠レイアウト自体を管理画面から編集する機能。
- スキル名生成にもプロバイダ自動フォールバックを効かせる(現状は画像生成のみ。テキスト生成はOpenAI固定)。

## Ver2.10追記3: カード合成をプレビュー時点に前倒し

「採用するをクリックしても採用後の画像が見えないので判断できない。カード生成→確認→採用の順が良い」という指摘を受けて対応。従来は「生成する」ボタンでAIの素のイラストのみプレビューし、「採用する」を押した後(adopt時)にレアリティ枠・武将名・スキル名・ステータス等を合成していたため、実際にどんなカードになるかは採用してからでないと確認できなかった。

### 変更内容

- マイグレーション`20260724000001_warlord_ai_portrait_url.sql`: `warlords`に`ai_portrait_url`(カード合成前の素のAIイラストURL)を追加。
- `/api/admin/ai-image/generate`: 武将カードの場合、AI生成イラストを取得した直後に`renderWarlordCard()`でカード合成まで行い、**合成済みの画像をプレビューとして返す**ようにした(`image_base64`)。素のイラストも`portrait_base64`として同時に返す。
- `/api/admin/ai-image/adopt`: カード合成はgenerate側で完了済みのため、adopt側では合成処理を行わず、受け取った画像をそのままリサイズ・保存するだけに簡略化。`portrait_base64`が渡された場合は、素のイラストを`ai_portrait_url`列にも別途保存する。
- 「現在の画像を参照する」の参照先を、武将カードのみ`image_url`(枠・文字が焼き込み済み)ではなく`ai_portrait_url`(素のイラスト)に変更した。理由: 既に枠・文字が合成された画像をAIへの参照画像として渡すと、その枠や文字までAIが模写しようとして再生成のたびに劣化する懸念があるため。`ai_portrait_url`が無い(過去に一度もAI生成を採用したことがない)場合は、従来どおり`image_url`にフォールバックする。
- `AiImageGeneratePanel.tsx`(共通コンポーネント): `portrait_base64`を状態に保持し、採用時に`image_base64`と一緒にadopt APIへ送るよう変更。

### 影響範囲

- 既存テーブルへの列追加のみ。武将以外のエンティティ(エリア・物件・シーン・全体マップ)の生成〜採用フローは変更なし(`portrait_base64`は常に`undefined`のため、既存の1枚だけの動作のまま)。

### 未実装事項

- 過去に採用した武将画像(`ai_portrait_url`が未設定のもの)は、次回「現在の画像を参照する」を使うと`image_url`(枠付き)にフォールバックする点は変わらず未解決(次にAI生成・採用すれば`ai_portrait_url`が設定され、以降は解消される)。

## Ver2.10追記4: 内覧画像の設定バグ修正+スタイル方向性の見直し

実際に全体マップ画像を生成したところ、平面的な2Dゲームマップ風のイラスト(色分けされた区画・記号的な建物アイコン)になってしまい、ユーザーの希望(山上の城から川沿いの城下町を見渡す、ゲームエンジンでレンダリングしたような奥行きのある俯瞰景観)とは大きく異なる結果になった。調査の結果、**プロンプト文言の問題だけでなく、設定読み込みロジックのバグ**が原因と判明。

### 見つかったバグ

`src/lib/ai-image-settings.ts`の`getAiImageSettings()`が`{ ...DEFAULT_SETTINGS, ...data }`という単純なオブジェクトスプレードで実装されていたため、Ver2.10追記(内覧画像用スタイルプロンプト分離)のマイグレーション適用後、**既存の設定行に対しては`metaverse_style_prompt_template`列がnullのまま**(新規追加した列のバックフィルをしていなかったため)であるにもかかわらず、そのnullがコード側のデフォルト値を上書きしてしまい、**内覧画像生成時に共通スタイルプロンプトが実質的に何も適用されない状態**になっていた。そのため、各生成画面ごとの個別プロンプト文言(全体マップの場合は「色分け・区画線を入れてください」)だけがそのまま反映され、平面的なゲームマップ風の結果になっていた。

### 変更内容

- `getAiImageSettings()`をnullセーフなマージに修正(DBの値が`null`/`undefined`の列は、コード側のデフォルト値にフォールバックするようにした)。これにより、今後同様の列追加を行った際に同じ事象が再発しなくなる。
- 内覧画像用スタイルプロンプトのデフォルト文言を、「フォトリアルを避ける」路線から「良質なゲームエンジンでレンダリングされた、奥行き・立体感のある3D空間」路線に修正。「平面的な2Dマップ図・アイコン風のイラストにはしない」ことを明記しつつ、「現代的な要素が写り込む実写の観光写真には見えないようにする」という歯止めは残した。
- 全体マップ画像の自動プロンプト(`src/app/admin/(dashboard)/metaverse/maps/page.tsx`)から「色分け・区画線を入れてください」という指示を削除し、代わりに「ドローン撮影のような壮大な俯瞰構図」「地形・街並みの奥行き・光と影」「ゲームエンジンでレンダリングしたような臨場感」を明記するよう変更。区画境界の色分けは、実際には管理画面側でポリゴンとして別途設定する仕組みが既にあるため、AI生成画像に含める必要性は薄いと判断した。

### 影響範囲

- 既存テーブルへの列追加・マイグレーションは不要(コード側の修正のみ)。ただし、この修正により内覧画像生成の実際の見た目が変わる(意図した変更)。

### 未実装事項

- 実際の生成結果が期待どおりの「ゲームエンジンレンダリング風」になるかは、モデル・プロンプト解釈に依存するため、ユーザー側での確認・追加の文言調整が必要。

## Ver2.10追記5: ペイロード肥大化エラーの修正+武将カードの明るさ調整

実際に武将カードを採用しようとしたところ、`Unexpected token 'R', "Request En"... is not valid JSON`というエラーが発生。これは「Request Entity Too Large」というプラットフォーム側のエラーレスポンス(JSONではなくプレーンテキスト)をフロントエンドがJSONとしてパースしようとして起きたもの。原因は、Ver2.10追記3(カード合成のプレビュー前倒し)で、`/api/admin/ai-image/generate`が「カード合成後の画像」と「素のイラスト」の2枚をPNGのままbase64化して返すようになったため、リクエスト/レスポンスボディがVercelのペイロード上限に達してしまったこと。

### 変更内容

- `/api/admin/ai-image/generate`: レスポンスに含める2枚の画像を、base64化する前にWebP(quality 85)へ軽量変換するようにした(`compressForTransit()`)。最終的な保存時はadopt側の`resizeForLine()`で改めて適切な形式に変換されるため、この変換が最終的な画質に影響することはない。ランダムノイズ画像での検証で、PNG→WebP変換によりサイズが約1/4になることを確認済み(実際の写真的な生成画像ではさらに圧縮率が高くなる見込み)。
- `AiImageGeneratePanel.tsx`: プレビュー画像の`<img>`タグが`data:image/png;base64,...`のまま固定されていたため、実体がWebPになったことに合わせて`data:image/webp;base64,...`に修正(データURIのMIME宣言と実際のバイト列が食い違うとブラウザがデコードに失敗するため)。
- 武将カードのデフォルトスタイルプロンプトに「画面全体が暗くなりすぎないようにする。人物の顔・表情・鎧の質感がはっきりと見える明るさを保つ」という指示を追加(生成結果が全体的に暗すぎるという指摘への対応)。既存の設定行では武将カード用スタイルプロンプトは既にバックフィル済み(nullではない)のため、この変更を反映するには管理画面から手動でテキストを更新する必要がある。

### 影響範囲

- マイグレーション不要(コード修正のみ)。生成プレビューのファイル形式がPNGからWebPに変わるが、保存される最終画像の形式(WebP、`resizeForLine()`による)は変わらない。

### 未実装事項

- 実際のAI生成画像(ランダムノイズではない)でのペイロードサイズの実測確認、明るさ調整後の生成結果の確認はユーザー側で実施が必要。

## Ver2.10追記6: 画像配信を全面的にプロキシ経由に変更(CDN不具合の根本対策)

「武将画像がCDN(Cloudflare/Supabase Storage)側のRangeリクエストキャッシュ不具合により崩れる」という、以前から`docs/FEATURES.md`の既知の未解決事項に記載していた不具合が、AI画像生成機能の参照画像アップロードで実機再現し、さらに「他の部分でも発生している」との報告を受けたため、根本対策を実施した。

### 原因

Supabase Storageの公開URL(`/storage/v1/object/public/...`)は、CDN層でRangeリクエストへの応答が誤ってキャッシュされ、以後そのURL全体が壊れて配信され続ける不具合がある。`cacheControl`を短く設定する緩和策では、キャッシュの有効期限が切れるまでの間は再発し続け、根本解決にはならない。

### 変更内容

- `src/app/api/storage-proxy/route.ts`(新規): `bucket`/`path`をクエリパラメータで受け取り、Supabase Storage SDKの認証済み`download()`(公開CDN層とは別のエンドポイントを経由する)で画像を取得し直して返す。対象バケットはホワイトリスト化(`warlord-images`/`metaverse-images`/`rich-menu-images`/`gacha-animations`)。
- `src/lib/image-url.ts`(新規)の`toDisplayUrl()`: Supabase Storageの公開URLを`/api/storage-proxy?bucket=...&path=...`形式のURLに変換するヘルパー。Supabase以外のURL(静的アセット等)やnull/undefinedはそのまま返す。
- アプリ内で画像を表示している全箇所(管理画面・LIFF・外部内覧ページ、計16ファイル)の`<img src={...}>`/SVGの`<image href={...}>`を、`toDisplayUrl()`を通すように変更。共通コンポーネント(`ImageSlot`、`ReferenceImageSlot`、`PolygonCanvas`)は内部で1箇所直せば済むようにした。
- **動画は対象外**: 動画はネイティブのシーク機能のためにRangeリクエストが本質的に必要で、プロキシ側でRange対応を実装するのは別途大きな作業になるため、今回のスコープからは意図的に外した(動画で同様の不具合が報告された場合は改めて対応する)。
- 既存のDB上の画像URL(公開CDN URLのまま)はそのまま。**マイグレーション・データ移行は不要**(表示時に変換するだけのため)。

### 影響範囲

- 表示経路が変わるのみで、アップロード・保存のロジックは変更なし。すべての画像が自前のAPIルートを1回経由するようになるため、直接CDNから配信する場合と比べてわずかにレイテンシが増える可能性があるが、体感できるほどではない見込み。
- プロキシ route が`SUPABASE_SERVICE_ROLE_KEY`を使うため、画像配信そのものにも管理者権限相当のAPI呼び出しが発生する。ただし返す内容は元々`public`バケットの画像でありアクセス制御上の変化はない。
