...
Phase B-1(PR3)で、Stripe webhook route(`src/app/api/stripe/webhook/route.ts`)のイベント処理本体(inboxの原子的claim・`purchases`のprocessing遷移・`runPurchaseGrant()`呼び出し)を`src/modules/commerce/application/process-stripe-webhook-event.ts`・`StripeInboxRepository`(`src/modules/commerce/infrastructure/supabase-stripe-inbox-repository.ts`)へ抽出した。`PurchaseRepository`へ`findByStripeSessionId`/`claimForProcessing`を追加した。ルート自体はStripe署名検証とHTTPレスポンス変換のみを行う薄い層になり、副次的にPhase B-2(API Route薄型化)の対象条件も満たす形になった。`external-orders.ts`・`external-order-notifications.ts`はまだ対象外(次PR以降で扱う)。
