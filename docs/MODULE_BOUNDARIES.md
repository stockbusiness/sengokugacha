...
Phase B-1(PR2)で、`src/lib/purchase-grants.ts`・`src/lib/purchases.ts`の実装本体を`src/modules/commerce/application/`(`run-purchase-grant.ts`、`PurchaseRepository`/`PurchaseGrantStepRepository`/`PurchaseOutboxGateway`/`UserRepository`インターフェースのみに依存)・`src/modules/commerce/infrastructure/`(`SupabasePurchaseRepository`等)へ分離した。`PurchaseOutboxGateway`は、outbox対象テーブルを所有するintegrationsモジュール側のPhase B-1対応(`IntegrationOutboxRepository`)が完了するまでの暫定的な薄いアダプタとして`src/lib/integration-outbox.ts`(バグ修正PR9)へ委譲している。

Phase B-1(PR3)で、Stripe webhook route(`src/app/api/stripe/webhook/route.ts`)のイベント処理本体(inboxの原子的claim・`purchases`のprocessing遷移・`runPurchaseGrant()`呼び出し)を`src/modules/commerce/application/process-stripe-webhook-event.ts`・`StripeInboxRepository`(`src/modules/commerce/infrastructure/supabase-stripe-inbox-repository.ts`)へ抽出した。`PurchaseRepository`へ`findByStripeSessionId`/`claimForProcessing`を追加した。ルート自体はStripe署名検証とHTTPレスポンス変換のみを行う薄い層になり、副次的にPhase B-2(API Route薄型化)の対象条件も満たす形になった。

Phase B-1(PR4)で、`src/lib/external-orders.ts`(767行)の実装本体を、元ファイルの7セクション構成(注文状態遷移・注文登録・紐付け・区画割当・区画権利付与・キャンセル/返金/権利取消・一覧/詳細)にそのまま対応する形で`src/modules/commerce/application/external-order-*.ts`(7ファイル)・`ExternalOrderRepository`(`src/modules/commerce/infrastructure/supabase-external-order-repository.ts`)へ分離した。このファイルにはDBトランザクションが存在しないため、元の「事前条件チェックを全て先に行い、各更新はガード付きupdateで再試行安全にする」設計はそのまま維持している。`external-order-notifications.ts`の`notifyExternalOrderEvent()`は、`castle-notifications.ts`の各関数と同様にSupabaseクライアントの受け渡しを呼び出し元に要求しない設計のため、application層から直接呼び出す既存の協調モジュールとして扱い、Phase B-1の対象には含めていない。
