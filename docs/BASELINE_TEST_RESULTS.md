...
本書はモジュール化の各PRが進むごとに更新する(現時点ではPR2「ガチャ・国取り」・PR5「代理店・共通ID」・PR8「決済・権利」・PR11「integrations」分の自動テストが完了)。
...
- `npx vitest run`: 121/121 pass(モジュール化PR2で2ファイル・17テスト、PR5で2ファイル・10テストを追加。PR8は既存テストの棚卸しのみで新規テスト追加なし。PR11で1ファイル・10テストを追加)
- `npm run build`: 成功
