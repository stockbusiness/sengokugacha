...
本書はモジュール化の各PRが進むごとに更新する(現時点ではPR2「ガチャ・国取り」・PR5「代理店・共通ID」・PR8「決済・権利」・PR11「integrations」・PR12「Stripe Webhook」・PR13「セッション基盤」・PR14「アーキテクチャ依存関係」分の自動テストが完了)。

## ガチャ・国取り(PR2で自動テスト化、PR3/PR4でdomain層へ移設済み)

| 対象 | テストファイル | 内容 |
|---|---|---|
| 排出率ティア選択 | `src/modules/gacha/domain/rate-tiers.test.ts`(PR14でdomain層へ移設済み) | `pickTierRates()`: 制圧数に応じたティア境界の選択、空リスト時のフォールバック |
...
## アーキテクチャ依存関係(PR14でCI化)

千ノ国パスポート モジュール化指示書 §14(アーキテクチャ依存関係ルール)。domain層
(`src/modules/*/domain/`)がSupabase/Next.js/Stripe等のインフラ・フレームワークに直接依存
していないことを`src/modules/architecture-rules.test.ts`で機械的に検証する。

このテスト追加の過程で、`src/modules/gacha/domain/draw-policy.ts`が`src/lib/gacha-rate-tiers.ts`
(Supabaseクライアントを直接importする`getGachaRateTiers()`を同じファイルに含む)経由で
`pickTierRates()`をimportしており、domain層がSupabaseに間接依存する状態になっていたことが
判明した。純粋関数`pickTierRates()`/`DEFAULT_TIERS`/型`GachaRateTier`を
`src/modules/gacha/domain/rate-tiers.ts`へ移設し、`draw-policy.ts`はそちらから直接importする
よう変更して解消した(`src/lib/gacha-rate-tiers.ts`は新モジュールからimport+re-exportし、
DB依存の`getGachaRateTiers()`のみを引き続き保持する)。

| 対象 | テストファイル | 内容 |
|---|---|---|
| domain層の依存関係ルール | `src/modules/architecture-rules.test.ts` | `src/modules/*/domain/`配下の全`.ts`ファイル(テストファイル除く)について、直接importの中に`next`/`@supabase/*`/`stripe`/Supabase・Stripeクライアント生成ラッパー(`@/lib/supabase-server`/`@/lib/stripe`)が含まれていないことを検証 |

### このテストが検証しない範囲(既知の限界)

このテストは各ファイル自身の**直接import**のみを検査し、import先がさらに何をimportしているか
(推移的な依存関係)までは解決しない。`src/modules/castle/domain/commission-engine.ts`が
`@/lib/agent-rank.ts`(DB非依存の純粋関数のみで構成、既存テスト済み)をimportしている箇所は
現状問題ないことを目視で確認済みだが、将来`src/lib/*`配下のファイルにDB依存処理が追加された
場合、この推移的依存は本テストでは検知できない。新規にdomain層へファイルを追加する際は、
import先がさらに何に依存しているかも目視で確認すること。

## 検証結果

- `rm -rf .next && npx tsc --noEmit`: エラーなし
- `npm run lint`: 既存の`<img>`警告2件のみ(本作業と無関係)
- `npx vitest run`: 150/150 pass(モジュール化PR2で2ファイル・17テスト、PR5で2ファイル・10テストを追加。PR8は既存テストの棚卸しのみで新規テスト追加なし。PR11で1ファイル・10テスト、PR12で1ファイル・5テスト、PR13で1ファイル・6テスト、PR14で1ファイル・18テストを追加)
- `npm run build`: 成功
