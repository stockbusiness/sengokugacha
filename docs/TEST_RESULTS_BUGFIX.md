...
| `rm -rf .next && npx tsc --noEmit` | エラーなし |
| `npm run lint` | 0 errors, 2 warnings(既存の`<img>`警告のみ、本作業と無関係) |
| `npx vitest run` | 152/152 pass(変更なし。DB依存のイベント処理・新規APIルートのみでunit test化可能な新規純粋ロジックは無い) |
| `npm run build` | 成功(新規ルート`retry-common-user-merges`/`unresolved-common-user-merges`が出力に含まれることを確認) |

### 未実施の検証

- `handleAssignedAgentUpdated()`で対象ユーザーが見つからない場合に`unresolved_agent_assignments`へ`user_not_found`として保存されること。
- `handleCommonUserMerged()`で統合元ユーザーが見つからない場合に`unresolved_common_user_merges`へ保存され、後日該当ユーザーが同期された時点で`retry-common-user-merges`から再解決できること。
- `retry-common-user-merges`実行後、解決した行が`/admin/integration-recovery`の一覧から消えること。

いずれもDB統合テスト環境(Supabase local等)が無いと実行結果としての確認はできていない。本番投入前に、実際の代理店連携イベント(またはcurlでの模擬イベント送信)を使った実地検証を推奨する。
...
| `rm -rf .next && npx tsc --noEmit` | エラーなし |
| `npm run lint` | 0 errors, 2 warnings(既存の`<img>`警告のみ、本作業と無関係) |
| `npx vitest run` | 152/152 pass(変更なし。外部副作用の記録経路変更のみでunit test化可能な新規純粋ロジックは無い) |
| `npm run build` | 成功(新規ルート`/api/admin/integration-outbox`・`/api/admin/integration-outbox/drain`が出力に含まれることを確認) |

### 未実施の検証

- 購入完了時、紹介confirm(sengoku-ai.com宛)・区画購入LINE通知が送信前に`integration_outbox_events`/`notification_outbox_events`へ記録されること。
- 送信成功時は`status='sent'`・`sent_at`が記録され、送信失敗時は`status='failed'`・`last_error`・`attempt_count`が記録されること。
- `/admin/integration-recovery`画面の「購入イベント外部送信」セクションで未送信・失敗イベントが一覧表示され、「全件再送を試行」で再送できること。
- 同一購入への`runPurchaseGrant()`再実行(再試行API経由)時、`enqueueOutboxEvent()`の冪等insert(unique制約+`23505`検知)により重複行が作られず、同一outbox行が再利用されること。
- `integration_outbox_events`への`source_type`/`source_id`列追加(NOT NULL)を本番適用する前に、既存行が0件であることの確認(マイグレーションのコメントに記載)。

いずれもDB統合テスト環境(Supabase local等)が無いと実行結果としての確認はできていない。本番投入前に、テストモードのStripe決済(紹介経由ユーザー・区画購入の両方)を実行し、outboxテーブルへの記録・管理画面からの再送を実地確認することを推奨する。
...
| `rm -rf .next && npx tsc --noEmit` | エラーなし |
| `npm run lint` | 0 errors, 2 warnings(既存の`<img>`警告のみ、本作業と無関係) |
| `npx vitest run` | 152/152 pass(変更なし。管理画面API・DB操作のみでunit test化可能な新規純粋ロジックは無い) |
| `npm run build` | 成功(新規ルート`/api/admin/entitlements/unresolved`・`/api/admin/entitlements/retry-resolve`・`/api/admin/entitlements/[id]/dismiss`が出力に含まれることを確認) |

### 未実施の検証

- `common_user_id`未解決のentitlementが`/admin/integration-recovery`画面の「未解決のentitlement」セクションに表示されること。
- 「全件再解決を試行」実行後、該当ユーザーの`common_user_id`同期が完了していれば残高付与まで完了し、一覧から消えること。
- 「却下」実行後、対象entitlementが一覧から消え、`resolution_dismissed_at`/`resolution_dismissed_by`/`resolution_dismissal_note`が記録されること。却下済み行への再度の「却下」呼び出しが409になること。
- operatorロールでの`retry-resolve`/`dismiss`呼び出しが403で拒否されること。

いずれもDB統合テスト環境(Supabase local等)が無いと実行結果としての確認はできていない。本番投入前に、`common_user_id`未解決のentitlementを意図的に作成(またはcurlでの模擬イベント送信)し、再解決・却下の一連の操作をmanager/operator両アカウントで実地確認することを推奨する。
